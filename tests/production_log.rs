...
fn for_each_fixture(mut check: impl FnMut(&str, usize, &LogEntry) -> Vec<String>) -> Vec<String> {
    let mut violations = Vec::new();
    for file in &fixture_files() {
        let name = file.file_name().unwrap().to_string_lossy();
        for (i, entry) in LogStream::new(lines_from_file(file)).enumerate() {
            violations.extend(check(&name, i, &entry));
        }
    }
    violations
}
...
fn no_execute_in_attached() {
    let violations = for_each_fixture(|name, i, entry| {
        let mut v = Vec::new();
        for (j, line) in entry.attached.iter().enumerate() {
            ...
            if parsed.kind == LineKind::UuidContinuation && parsed.message.starts_with("EXECUTE ") {
                v.push(format!("{name}: entry {i} attached[{j}]"));
            }
        }
        v
    });
    ...
    let violations = for_each_fixture(|name, i, entry| {
        if entry.message_kind == MessageKind::ChannelData && !entry.attached.is_empty() {
            if entry.block.is_none() {
                return vec![format!(
                    "{name}: entry {i} CHANNEL_DATA with {} attached but no block",
                    entry.attached.len()
                )];
            }
            if let Some(Block::ChannelData { fields, variables }) = &entry.block {
                if fields.is_empty() && variables.is_empty() {
                    return vec![format!(
                        "{name}: entry {i} CHANNEL_DATA block with empty fields and variables"
                    )];
                }
            }
        }
...
fn sdp_has_typed_block() {
    let violations = for_each_fixture(|name, i, entry| {
            ...
            && !entry.attached.is_empty()
        {
            if entry.block.is_none() {
                return vec![format!(
                    "{name}: entry {i} SDP marker with {} attached but no block",
                    entry.attached.len()
                )];
            }
            if let Some(Block::Sdp { body, .. }) = &entry.block {
                if body.is_empty() {
                    return vec![format!("{name}: entry {i} SDP block with empty body")];
                }
            }
        }
        vec![]
    ...
    for file in &fixture_files() {
        ...
        for entry in stream.by_ref() {
            ...
            *entry_line_kind_counts
            ...
            if message_kind_label(&entry.message_kind) == "general" && general_samples.len() < 20 {
