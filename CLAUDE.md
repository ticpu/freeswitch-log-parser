...
- `initial_context`, `initial_destination`, `initial_caller_number`, `initial_caller_name` — the first `Processing` line's values, set once so a transfer's feature-token routing cannot restate them. First *seen*: a window opening mid-call pins whatever line arrived first in it. Only a bracketed caller side yields the two caller halves, matching the field span surface; the name comes through the prefix stage's re-encode and is a rendering, not the bytes
- `caller_number()`/`callee_number()` — the precedence ladders, dump field then `sip_from_user`/`sip_to_user` then the dialplan. On `SessionState` only: `SessionSnapshot` carries no variables and cannot walk the middle rung
